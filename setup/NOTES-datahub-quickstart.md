...
- **Recipe**: `setup/ingest_postgres.yml` — postgres source (`localhost:5434`, db `demo_warehouse`, schema_pattern allow `ecommerce`, table-level metadata + schemas), `datahub-rest` sink to `http://localhost:8080`. Profiling left off — a one-line flag exists (`profiling.enabled: true`) but it runs per-table/per-column stats queries for no payoff in this project (nothing reads profiling stats), so skipped rather than adding runtime for nothing.
- **Command that worked**:
  ```
